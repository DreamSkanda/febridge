"""Checkpoint utilities for febridge."""
...
def find_ckpt_filename(path_or_file: str) -> Tuple[Optional[str], int]:
    """Finds the latest checkpoint file in a directory or validates a specific file.

    Args:
        path_or_file: Directory path or specific checkpoint file path.

    Returns:
        Tuple of (filename, epoch) or (None, 0) if no valid checkpoint found.
    """
...
def load_data(filename: str):
    """Loads checkpoint data from a pickle file.
...
def ckpt_hyperparams(filename: str) -> Tuple[Tuple[int, int, float], Tuple[str, int, int, Optional[int]]]:
    """Extracts hyperparameters from a checkpoint filename.

    Args:
        filename: Checkpoint filename.

    Returns:
        Tuple of ((n, dim, beta), (net, nlayers, nh, nk)).
    """
